...
*** Comments
A comment is a string of characters beginning with "/*" and ending
with "*/" which can contain other comments, but not unmatched occurrences
...
A /formula/ in general is a basic formula or one of several kinds of compound
formulas.  
...
| Smullyan   | ⁓      | ∧       | ∨    | ⊃           | ↔        | ∀x     | ∃x     |
| Quine 1950 | —, p̄   |         | ∨    | →           | ↔        | ∀x     | ∃x     |
| Alloy      | not, ! | and, && | or* ​ | implies, => | iff, <=> | all x  | some x |
| TLA+       | ¬      | ∧       | ∨    | ⇒           | ≡        | ∀x     | ∃x     |
| TLA+ ASCII | ~      | /\      | \/   | ﻿=>          | <=>      |        |        |
|            | \lnot  | \land   | \lor |             | \equiv   |        |        |
|            | \neg   |         |      |             |          |        |        |
| Agda ASCII | \lnot  | \and    | \or  |             |          |        |        |
|            | \neg   | \wedge  | \vee |             |          |        |        |
|------------+--------+---------+------+-------------+----------+--------+--------|
