...
all = universal-quantifier, bracketed-formula.
exist = existential-quantifier, bracketed-formula.
one = unique-existential-quantifier, bracketed-formula.
none = negated-existential-quantifier, bracketed-formula.
lone = lone-existential-quantifier, bracketed-formula.
...
On descriptions see Quine /Methods/ 274-282 (or so).

#+begin_src ixml
the = "℩" { #2129 turned Greek small letter iota }
    | "the"
    .
#+end_src
...
- All negation to bind more tightly than the binary operators?
